...
function MainController ($scope, $location) {
   ...
   $scope.entityLongClick = function ($event, page, item, entity) {
      ...
      switch (item.type) {
         ...
         case TYPES.CLIMATE: return $scope.openClimateSelects(item, entity);
      }
   ...
   $scope.entityLeftState = function (item, entity) {
     
    if(item.lstate === false) return null;

    var res;

    if(item.lstate) {
        if(typeof item.lstate === "string") {
            return parseString(item.lstate, entity);
        }
        else if(typeof item.lstate === "function") {
            res = callFunction(item.lstate, [item, entity]);
            if(res) return res;
        }
    }

    if(item.lstates) {
        if(typeof item.lstates === "function") {
            res = callFunction(item.lstates, [item, entity]);
        }
        else if(typeof item.lstates === "object") {
            res = item.lstates[entity.state] || entity.state;
        }

        if(res) return res;
    }

    if(!item.lstate) return entity.lstate;
   ...
   $scope.getClimateSelectValue = function (select, conf) {
    if(select.formatValue) return select.formatValue(conf);

    return conf.value;
};

$scope.openClimateSelects = function (item, entity) {
 if(entity.state === "off") {
    return $scope.toggleSwitch(item, entity, function () {
       setTimeout(function () {
          if(entity.state !== "off") {
             $scope.openClimateSelects(item, entity);
             updateView();
          }
       }, 0);
    })
...
$scope.closeClimateSelects = function ($event, item, entity) {
 $event.preventDefault();
 $event.stopPropagation();
 ...
 return false;
   ...
   $scope.setClimateFan = function ($event, item, entity, option) {
 ...
 $scope.setClimateSwing = function ($event, item, entity, option) {
   ...
   $scope.toggleClimate = function (item, entity, callback) {
    var service = "toggle";
    if(entity.state === "off") service = "turn_on";
    else service = "turn_off";
    sendItemData(item, {
        type: "call_service",
        domain: "climate",
        service: service,
